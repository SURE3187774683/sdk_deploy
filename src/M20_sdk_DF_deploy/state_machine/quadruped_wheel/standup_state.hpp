...
private:
    ...
    const float set_wheel_kp_ = 8.;
    const float set_wheel_kd_ = 4.;
...
public:
    ...
    virtual void OnEnter() {
        ...
        RecordJointData();
        for(int i=0;i<4;++i){
            wheel_lock_pos_(i) = current_joint_pos_(i*4+3);
        }
        ...
        if(run_time_ - time_stamp_record_ <= stand_duration_){
            for(int i=0;i<current_joint_pos_.rows();++i){
                ...
                planning_joint_vel(i) = GetCubicSplineVel(init_joint_pos_(i), init_joint_vel_(i), goal_joint_pos_(i), 0, 
                                                run_time_ - time_stamp_record_, stand_duration_);
                if(i%4==3){
                    planning_joint_pos(i) = wheel_lock_pos_(i/4);
                    planning_joint_vel(i) = 0;
                    kp_(i) = set_wheel_kp_;
                    kd_(i) = set_wheel_kd_;
                }
            }

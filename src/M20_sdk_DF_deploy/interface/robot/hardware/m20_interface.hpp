    ...
    void ResetPositionOffset() {
        ...
        while (rclcpp::ok() && !IsDataUpdatedFinished()) {
            ...
            if (cnt == 10000) {
                ...
                std::cerr << "[M20Interface] Did not receive /JOINTS_DATA in 10s. "
                          << "Check simulator is running and ROS_DOMAIN_ID matches." << std::endl;
                throw std::runtime_error("No joint data received from /JOINTS_DATA");
            }
        }

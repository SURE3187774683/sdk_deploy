...
private:
    ...
    // Keep `s`/`e` as mode keys for waypoint tracking start/stop.
    const std::unordered_set<char> velocity_keys_ = {'w', 'a', 'd', 'q'};
    ...
    void process_mode_command(char k)
    {
        if (k == 'r') {
            ...
            usr_cmd_->reserved_scale = 0.0f;
            usr_cmd_->target_mode = uint8_t(RobotMotionState::RLControlMode);
            std::cout << "[MODE] RL Control\n";
        }
        // Start point-tracking (alias of entering RL control) after full standing.
        else if (k == 's' && msfb_->GetCurrentState() == RobotMotionState::StandingUp) {
            usr_cmd_->reserved_scale = 0.0f;
            usr_cmd_->target_mode = uint8_t(RobotMotionState::RLControlMode);
            std::cout << "[MODE] Start Point Tracking\n";
        }
        // In RL, pause point tracking and recover to standing mode smoothly.
        else if (k == 'p' && msfb_->GetCurrentState() == RobotMotionState::RLControlMode) {
            usr_cmd_->target_mode = uint8_t(RobotMotionState::StandingUp);
            std::cout << "[MODE] Pause Point Tracking -> Standing Up\n";
        }
        // From standing, resume tracking and skip to the next waypoint.
        else if (k == 'p' && msfb_->GetCurrentState() == RobotMotionState::StandingUp) {
            usr_cmd_->reserved_scale = 1.0f;
            usr_cmd_->target_mode = uint8_t(RobotMotionState::RLControlMode);
            std::cout << "[MODE] Resume Point Tracking -> Next Point\n";
        }
    ...
    void keyboard_loop()
    {
        ...
        std::cout << "\n╔════════════════════════════════════════════════╗\n"
                  ...
                  << "╚════════════════════════════════════════════════╝\n"
                  << "  Movement:  W (forward)  A/D (left/right)\n"
                  << "  Rotation:  Q (CCW)\n"
                  << "  Mode:      R (damping)  Z (stand)\n"
                  << "             S (start point tracking)\n"
                  << "             P (pause->stand / resume next point)\n"
                  << "\n";
